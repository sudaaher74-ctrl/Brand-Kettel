...
const leadValidation_1 = require("../lib/leadValidation");
...
router.post('/', async (req, res) => {
    const parsed = leadValidation_1.ContactSubmissionSchema.safeParse(req.body);
    if (!parsed.success) {
    ...
    if (process.env.SMTP_HOST && process.env.SMTP_USER) {
        try {
            ...
            <p><strong>Name:</strong> ${(0, leadValidation_1.escapeHtml)(lead.name)}</p>
            <p><strong>Email:</strong> ${(0, leadValidation_1.escapeHtml)(lead.email)}</p>
            <p><strong>Phone:</strong> ${(0, leadValidation_1.escapeHtml)(lead.phone)}</p>
            <p><strong>Project Type:</strong> ${(0, leadValidation_1.escapeHtml)(lead.projectType)}</p>
            <p><strong>Message:</strong><br/>${(0, leadValidation_1.escapeHtml)(lead.message || 'No message provided').replace(/\n/g, '<br/>')}</p>
          `
            });
