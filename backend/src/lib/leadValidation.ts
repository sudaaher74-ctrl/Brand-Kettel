...
export const ContactSubmissionSchema = z
  .object({
    name: z
      .string()
      .transform(sanitize)
      .pipe(z.string().min(2, 'Name is too short').max(100, 'Name is too long')),
    phone: z
      .string()
      .max(30)
    ...
    email: z
      .string()
      .transform(sanitize)
      .pipe(z.string().max(254).email('Enter a valid email address'))
      .transform((v) => v.toLowerCase()),
    projectType: z.enum(PROJECT_TYPES, { message: 'Select a valid project category' }),
    message: z
      .string()
